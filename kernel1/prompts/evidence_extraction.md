...
- `quotes` 只能是对象数组，禁止输出字符串数组。错误示例：`"quotes":["原话"]`；正确示例：`"quotes":[{"quote":"原话","indicator":"IND TM-A",...}]`。
...
- `quotes` 输出 **15-30 条原始候选证据**（允许同一元素出现多次，要求内容多样性）。
- **禁止同一句原文出现多次**：每条 quote 的原文内容必须不同，不准把长句切片成多条（宁可 quote 字数超标也不准切片）。
- 不需要在这里平衡维度——后续会有整合环节；你的任务是**广采集**，把文本里所有能找到的 Socionics 相关行为证据都提取出来。
- 尽量覆盖 N/S/T/F 八个元素，若某元素无证据，在 `insufficiency` 里标注。
- 每条 `quote` 不超过 70 个中文字符。
